...
namespace SuretyTrust.Common.GenericSQL
{
	public partial class GenericSQL
	{
		...
		public List<Dictionary<string, GnSqlRowItem>> SchemaRead(string selectQuery, List<SqlParameter> lSqlParams = null)
		{
			if (!selectQuery.ToLower().Contains("select"))
			{
				throw new GenericSQLException("SchemaRead Failure: Select statement is required.");
			}
			...
			try
			{
				result = ExecuteSchemaRead(selectQuery, DbAction.read, lSqlParams);
			}
			catch (GenericSQLException ex)
			{
				...
				{
					Arguments = new List<object>{ selectQuery, lSqlParams },
					ErrorMessage = "ExecuteSchemaRead FAILED -> Error_Info: " + ex.ErrorInfo + " inner message: " + (hayInner ? ex.InnerException.Message : "N/A"),
					ErrorException = ex
				});
			}
			catch (Exception ex)
			{
				...
				GenericSQLException gnSqlException = new GenericSQLException("ExecuteSchemaRead FAILED", ex);

				throw _logger.LogErrorWithOptions(new ErrorOptions
				{
					Arguments = new List<object> { selectQuery, lSqlParams },
					ErrorMessage = "ExecuteSchemaRead FAILED -> Error_Info: " + gnSqlException.ErrorInfo + " inner message: " + (hayInner ? gnSqlException.InnerException.Message : "N/A"),
		...
		private List<Dictionary<string, GnSqlRowItem>> ExecuteSchemaRead(String sQuery, DbAction action = DbAction.read, List<SqlParameter> lSqlParams = null)
		{
			SqlCommand db;
